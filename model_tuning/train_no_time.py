...
CONFIG = {
    ...
    "epochs": 5,
    "warmup_ratio": 0.1,
    "output_dir": str(Path(__file__).resolve().parents[1] / "src" / "model" / "results" / "checkpoints"),
    "data_dir": str(Path(__file__).resolve().parents[1] / "model_tuning" / "data" / "processed"),
    "seed": 42,
}
...
def set_seed(seed):
    """Seed all RNGs used during training for reproducibility across ablation runs."""
    random.seed(seed)
    np.random.seed(seed)
